...
def polynomialize(vec):

    return np.asarray([vec[0], vec[1], (vec[0] * vec[1]), np.log(vec[1] * vec[1]), np.exp(vec[0])])

def polynomial(va, vb, sigma):
    return np.dot(polynomialize(va), polynomialize(vb)) * sigma
...
def solve_dual(X, Y, gamma, sigma):
    ...
    for i in xrange(N):
        ...
        for j in xrange(N):
            # print (i, j)
            a_sum.add(a[i] * a[j] * polynomial(X[i], X[j], sigma) * Y[i] * Y[j])
...
def sign(theta_X, theta_Y, item, sigma):
    if polynomial(np.asarray([theta_X, theta_Y]), np.asarray([item[0], item[1]]), sigma) > 0:
        return 1
    else: return -1
